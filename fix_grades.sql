-- ============================================
-- FIX GRADES - Standalone SQL Script
-- Run this script to recalculate all grades with the correct grading system
-- ============================================

-- Step 1: Create function to calculate grade from percentage
...
-- Step 2: Create function to calculate total percentage from scores
...
-- Step 3: Add grade column if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
    ...
    RAISE NOTICE 'Added grade column to grades table';
  ELSE
    RAISE NOTICE 'Grade column already exists';
  END IF;
END $$;

-- Step 4: Update all existing grades with correct calculations
...
BEGIN
  ...
  SELECT 
    ...
    calculate_total_percentage(g.project, g.test1, g.test2, g.group_work, g.exam) as total_percentage,
    g.grade
  INTO example_record
  FROM public.grades g
  JOIN public.students s ON g.student_id = s.id
  JOIN public.subjects sub ON g.subject_id = sub.id
  WHERE ABS(calculate_total_percentage(g.project, g.test1, g.test2, g.group_work, g.exam) - 67.5) < 0.1
  LIMIT 1;
  
  IF FOUND THEN
    RAISE NOTICE '';
    RAISE NOTICE 'Found example record:';
    RAISE NOTICE '  Student: %', example_record.student_name;
    RAISE NOTICE '  Subject: %', example_record.subject_name;
    RAISE NOTICE '  Total: %', example_record.total_percentage;
    RAISE NOTICE '  Grade: %', example_record.grade;
  END IF;
  
  RAISE NOTICE '========================================';
END $$;

-- Step 6: Create trigger to automatically update grade when scores change (optional but recommended)
...
RAISE NOTICE 'Trigger created: Grades will now be automatically recalculated when scores change';

-- Step 7: Add index on grade column for better query performance
CREATE INDEX IF NOT EXISTS idx_grades_grade ON public.grades(grade);

RAISE NOTICE 'Index created on grade column';
